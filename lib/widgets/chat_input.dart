...
class _ChatInputState extends State<ChatInput> {
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: IconButton(
            ...
            icon: _uploadedItems.any((item) => item.isProcessing) || provider.isResponding
              ? SizedBox(
                  width: 24,
                  height: 24,
  ...
  bool get _canSend {
    ...
    if (_uploadedItems.any((item) => item.isProcessing) || provider.isResponding) return false;
    ...
    try {
      ...
      if (file != null) {
        ...
        Timer.periodic(const Duration(milliseconds: 16), (timer) {
          if (!mounted || !uploadedItem.isProcessing) {
            timer.cancel();
            return;
          }
          setState(() {
            if (uploadedItem.processProgress < 0.9) {
              uploadedItem.processProgress += (0.9 - uploadedItem.processProgress) * 0.05;
            }
          });
        });
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    final provider = context.read<ChatProvider>();
                    provider.removeUploadedItem(item);  // 先从 provider 中移除
                    setState(() {
                      _uploadedItems.removeAt(index);  // 再从本地状态移除
                    });
                  },
                ),
              ],
